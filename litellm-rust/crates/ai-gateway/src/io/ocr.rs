...
pub async fn ocr(request: OcrRequest<'_>) -> CoreResult<Value> {
    ...
    let dispatch = OcrProviderDispatch::for_provider_and_model(request.custom_llm_provider, model)
...
mod tests {
    ...
    fn ocr_dispatch_supports_migrated_providers() {
        assert_eq!(
            OcrProviderDispatch::for_provider_and_model("mistral", "mistral-ocr-latest"),
            Some(OcrProviderDispatch::Mistral)
        );
        assert_eq!(
            OcrProviderDispatch::for_provider_and_model("azure_ai", "pixtral-12b-2409"),
        ...
        assert_eq!(
            OcrProviderDispatch::for_provider_and_model("vertex_ai", "deepseek-ocr-maas"),
            Some(OcrProviderDispatch::VertexAiDeepSeek)
        );
        assert_eq!(
            OcrProviderDispatch::for_provider_and_model("openai", "gpt-4o"),
            None
        );
    }
