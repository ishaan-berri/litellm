...
impl OcrProviderDispatch {
    pub(super) fn for_provider_and_model(provider: &str, model: &str) -> Option<Self> {
        match provider {
            "mistral" => Some(Self::Mistral),
            "azure_ai" if is_azure_document_intelligence_model(model) => {
                Some(Self::AzureDocumentIntelligence)
            }
            "azure_ai/doc-intelligence" => Some(Self::AzureDocumentIntelligence),
            "azure_ai" => Some(Self::AzureAi),
            "vertex_ai" if vertex_ai::is_deepseek_model(model) => Some(Self::VertexAiDeepSeek),
            "vertex_ai" => Some(Self::VertexAiMistral),
            _ => None,
        }
    }
